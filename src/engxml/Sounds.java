...
public class Sounds {
	...
	public static void main(String[] args) throws IOException {
		...
		for (String word : words) {
			...
			if (word.startsWith("#")) {
				if (!phrases.isEmpty()) {
					units += (units.isEmpty() ? "\n" : ",\n") + String.format(f1, unit, unitTitle, unitPath, phrases.substring(1, phrases.length() - 1));
				}
				unit = word.split("#")[1];
		...
		Files.copy(new ByteArrayInputStream(jsonText.getBytes()), Paths.get(trgJsonFile), StandardCopyOption.REPLACE_EXISTING);
