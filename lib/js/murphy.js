...
Murphy.prototype.createControl = function(accordionID) {
	...
	$(this.playUnitsButton).audioClick();
...
Murphy.prototype.fillUnit = function(el, key, data) {
	...
	$.each(data.data, function(index, val) {
		...
		var row = $('<span>').attr("style",
				"font-size:300%;font-weight:bold;cursor:pointer;").text(
				val.phrase);
		$(row).audioStopClick();
		...
		var playSectionButton = Murphy.createPlayButton();
		tr.append(td1);
		tr.append(td2);
		td2.append(playPhraseButton);
		td2.append(playSectionButton);
		td1.append(row);
		var phraseSources = [];
		var sectionSources = [];
		...
		wordsRow.data("sound_sources", unitSources);
		$(wordsRow).audioClick();
		td1.append(wordsRow);
		var wordsText = "";
		...
		playSectionButton.data("sound_sources", sectionSources);
		$(playSectionButton).audioClick();
	});
